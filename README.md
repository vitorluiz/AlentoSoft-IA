...
Se a API retornar `Provider HTTP 429`, observe o tipo exibido entre parênteses. `insufficient_quota` normalmente indica ausência de saldo, orçamento mensal atingido ou API não habilitada para a organização; `rate_limit_exceeded` indica limite temporário de requisições ou tokens por minuto. O provider preserva o tipo e a mensagem pública devolvidos pela API, mas nunca inclui a chave. Consulte o painel de Usage Limits e billing da plataforma API antes de repetir muitas vezes. A OpenAI recomenda exponential backoff para limites temporários [1].
